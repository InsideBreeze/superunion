...
export default function CargoSearch() {
    ...
    const handleSearch = async () => {
        ...
        //
        const response = await axios.post(`http://api.superunionlogistics.com:8000/tracking`, `tracking_number=${code}`, {
            headers: {
 ...
 border-[var(--blue)] bg-transparent pl-[14px] text-xl outline-none placeholder:text-[#646464]"
                            ...
                            placeholder="Search"
                        />
                        <button onClick={handleSearch} disabled={code.trim().length === 0} className='bg-[red]'>
                            <Image
                                src="/images/rec.png"
                                width={60}
                                height={60}
                    ...
                    <AnimatePresence>
                        {history.length > 0 && code.length === 0 && (
                            <motion.div
            ...
            {notFound && (
                <p className='text-2xl text-center mt-10 text-[red]'>Not Found, please check your input :) </p>
            )}
        </div>
    )
}
